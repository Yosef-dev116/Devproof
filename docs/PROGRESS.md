...
- Build the frontend "Team Analysis" page (per the task spec): org name input → leaderboard (rank, developer, commits, engineering score, quality score, AI slop score, documentation, testing) → click a contributor for the full detailed report. Deliberately not started yet — sequenced as its own feature after the backend it depends on.
- No caching/persistence on the org engineering-report endpoint — re-running the same org re-pays every OpenAI call. Fine for v1 (same trade-off as resume-check), worth reconsidering once managers are actually re-checking the same org repeatedly.
...
~~Build the "actual code analysis" half of the team pivot~~ — done, see "Current Feature" above.
...
| Frontend UI for team/org analysis | Yosef | Not started | — |
