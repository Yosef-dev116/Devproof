...
**Current Feature**
- Third piece of the pivot, and the one that makes the previous two actually usable: the **Team Analysis frontend page**. New "Team Analysis" card in `frontend/src/App.tsx`: enter a GitHub org name → "Analyze Organization" (with a loading message, since a full org run can take up to a minute) → a contributor leaderboard table (rank, developer with avatar, commits, engineering score, quality score, type safety, documentation, testing) → clicking any row opens a full "Contributor Engineering Report" card above the leaderboard (overall score, summary, risk flags, all 7 category scores, strengths/weaknesses in two columns, and the list of repositories that were actually analyzed for them) with a "Back to leaderboard" button.
- Driven by a task spec (`Claude_Task.txt`) that explicitly said: reuse the existing `GET /organizations/{org_name}/engineering-report` endpoint as-is, don't touch the backend unless a real bug is found, and reuse existing styling wherever possible. No backend changes were made or needed. Every visual element reuses existing CSS classes/patterns from the single-repo report (`.card`, `.score-badge` tiers, `.table-scroll`/`.repo-table`, `.report-columns`, `.plain-list`, `.overall-score`) rather than inventing a parallel design system - the only genuinely new CSS is a handful of small additions (a neutral "N/A" score-badge variant, a clickable-row hover style for the leaderboard, and a couple of layout helpers for the contributor identity/score-row list).
- One adaptation from the task's example column list: it named "Quality Score" and "AI Slop Score" as if they were two separate columns, but the backend only returns one `average_quality_score` field (the AI's holistic "Code Quality & Type Safety (AI Slop)" category) plus a separately-computed `average_type_safety` (a raw, mechanical typed-code ratio, not AI-judged). The leaderboard uses the fields that actually exist rather than inventing a column with no real data behind it - "Quality Score" and "Type Safety" are shown, matching what step 2 of the pivot actually produces.
- New `ScoreBadgeOrEmpty` helper component handles the `null`-score case (insufficient evidence) everywhere a contributor's score is displayed - shows a neutral grey "N/A" badge instead of a colored score, so a missing score never looks like a real (if low) one.
- Verified: `tsc -b && vite build` clean; full flow tested live against the real deployed app (Vercel + Render) - entered a real org, watched the leaderboard populate with real scores, clicked into a contributor's full report, used "Back to leaderboard", and confirmed the existing repository-analysis and resume-verification flows on the same page still work unchanged.

**Two Features Ago**
- Second piece of the pivot: `GET /organizations/{org_name}/engineering-report` — the "actual code analysis" the previous feature deferred. For an org's top 10 contributors by commits, finds the repos each meaningfully contributed to (≥5 commits), dedupes that set across all 10 contributors (capped at ~15 unique repos so cost/latency stay bounded for a single request), and runs the **existing, unmodified** single-repo AI analysis pipeline once per unique repo — then aggregates each contributor's own scores from the repos they touched.
- **Refactor to enable reuse** (the one refactor in this feature, justified by "don't duplicate the analysis logic"): extracted the evidence-gathering + `generate_report()` call that used to live inline in `analyze_repository` (the `/repositories/{id}/analyze` route) into new `backend/app/repo_analysis.py::analyze_repo(owner, repo, repo_info)`. The existing route now just calls this function — identical behavior, verified by keeping the exact same error handling/messages. This is what lets the new org-report route reuse the exact same pipeline without copy-pasting it.
- New aggregation logic in `backend/app/team_analysis.py`: `select_top_contributors()`, `find_meaningful_contributions()` (the ≥5-commit floor), `select_repos_to_analyze()` (dedupe + cap, most-referenced-repo-first), and `build_contributor_engineering_profile()` — the core aggregator. All pure functions, no I/O, no OpenAI calls of their own; they only do arithmetic over already-computed AI report output.
- Per-contributor fields: `documentation_score`/`testing_score`/`devops_score`/`security_score`/`project_maturity_score` (averaged from the matching named category across their repos), `average_quality_score` (the AI's holistic "Code Quality & Type Safety (AI Slop)" category, averaged), `average_type_safety` (a **separate, purely mechanical** number — the typed-function/typed-file ratio computed directly from `detect_type_safety_signals()`'s raw counts, no AI judgment involved), `overall_engineering_score` (averaged `overall_score`), deduped `strengths`/`weaknesses` pulled directly from the underlying repo reports, rule-based `risk_flags` (e.g. low testing score, low type safety, single-repo evidence), and a templated `summary` sentence. Nothing here is AI-generated at the aggregation step — it's all arithmetic/rules over evidence the existing pipeline already produced, so it can't invent a claim that isn't backed by a real repo report.
- **Insufficient-evidence handling**: if a top-10 contributor's meaningful repos all got cut by the cap (or all failed to analyze), they still appear with their real commit count but every score is explicit `null` plus a risk flag saying there wasn't enough evidence in this run — never a fabricated number.
- 6 new unit tests (25/25 total now), all pure-function, no network/OpenAI calls: top-contributor selection/limiting, the commit floor, repo dedupe/cap, score averaging math, the insufficient-evidence path, and risk-flag rules.
- **Verified live with real data** (not just unit tests): ran the full pipeline against the real `psf` org with contributor/repo caps intentionally shrunk for the test (to bound real OpenAI spend) — confirmed real AI category names match the aggregation code's expectations exactly, confirmed a repo shared by two different top contributors (`requests`, touched by both `kennethreitz` and `Lukasa`) was correctly deduped and analyzed only *once* (not twice), confirmed multi-repo score averaging is correct, and confirmed risk flags fire correctly (e.g. `vstinner`'s single-repo `pyperf` contribution correctly flagged both low type safety and "limited evidence").
- **Not yet done:** no frontend UI (per the task spec, that's the deliberately separate next step). Stateless — no persistence, so re-running the same org re-triggers every OpenAI call again (same trade-off accepted for `/resume-check`). Not yet tested against the live Render deployment specifically (verified locally against real GitHub/OpenAI APIs).

**Previous Feature**
- First piece of the pivot: `GET /organizations/{org_name}/contributors` — takes a GitHub org name, fetches every repository in that org (paginated, so it's accurate for large orgs), and returns contributors aggregated with their total commit counts *across all of the org's repos*, sorted highest-first. Forked repos are excluded (a fork's commit history belongs to the upstream project, not the org, and would misattribute outside contributors' work as the team's own). Bot accounts (`type == "Bot"`, e.g. dependabot) are excluded from contributor counts. New `backend/app/team_analysis.py` (`aggregate_contributors` — pure function, no I/O, same pattern as `scoring.py`/`analysis.py`) with 4 new unit tests. `github_client.py` gained `list_org_repos()` and `fetch_repo_contributors()`/`fetch_contributors_for_repos()` (paginated, parallel across repos via the existing `ThreadPoolExecutor` pattern).
...
**Public deployment (Render + Vercel), prompted by wanting a live phone-accessible link for the pitch instead of presenting from a laptop:**
- Switched `backend/app/database.py` from SQLite to Postgres (`psycopg2`, `DATABASE_URL` env var) — required because hosted/serverless backends can't rely on a persistent local file the way `uvicorn --reload` on a dev machine could. Same function signatures, same behavior (upsert via `ON CONFLICT`, `RETURNING id` instead of `cursor.lastrowid`, `RealDictCursor` instead of `sqlite3.Row`). All 15 existing pure-function tests still pass unchanged (they never touched the database layer). Verified imports/syntax and a full frontend production build; a live Postgres round-trip will be verified against the real Render database once provisioned (no local Postgres/Docker available in this session to test against first).
- `backend/app/auth.py` and `main.py`: OAuth redirect URI, post-login redirect, CORS `allow_origins`, and session/oauth-state cookie `SameSite`/`Secure` flags are now all environment-driven (`BACKEND_URL`, `FRONTEND_URL`, `FRONTEND_ORIGINS`, `ENVIRONMENT`) instead of hardcoded to `localhost` — a cross-domain deployment (Vercel frontend, Render backend) needs `SameSite=None; Secure` cookies, which plain `http://localhost` cannot use, so this is conditional on `ENVIRONMENT=production` rather than a blanket change.
- New `backend/app/rate_limit.py`: a simple in-memory per-user, per-hour cap (`enforce_rate_limit`), applied to `/repositories/{id}/analyze` and `/resume-check` (20/hour each) — since a public link means strangers' usage costs real OpenAI/GitHub API budget, not just your own. Deliberately in-memory (not a DB table) for speed; only correct with a single backend instance, which matches Render's free tier.
- `frontend/src/App.tsx`: `API_BASE` now reads `import.meta.env.VITE_API_BASE_URL` (falls back to `localhost:8002` if unset) so the same build can point at a deployed backend.
- Existing untracked root `Dockerfile`/`.dockerignore` (already in the working tree, not yet committed) reused for the Render deploy — fixed its `CMD` to bind to Render's `$PORT` env var (shell form, not JSON/exec form, so `$PORT` actually expands) instead of a hardcoded `8002`.
- New `render.yaml` Blueprint (web service + free Postgres, most secrets marked `sync: false` so Render prompts for them rather than needing them committed) and new `docs/DEPLOYMENT.md` — full step-by-step for the account-level actions (Render/Vercel project creation, a **second** GitHub OAuth App for the production callback URL, since a single OAuth App can't hold two callback URLs and shouldn't take down local dev to get one).
- **Live and verified (2026-07-30):** backend deployed on Render at `https://devproof-api-7avo.onrender.com` (Postgres provisioned via the blueprint), frontend deployed on Vercel at `https://devproof-xi.vercel.app`. Second production GitHub OAuth App created and wired in. Confirmed via curl: `/health` returns `200`, `/auth/github/login` redirects to GitHub with the correct production `client_id`/callback, CORS preflight correctly allows the Vercel origin with credentials. Confirmed by the user directly on a phone: real "Sign in with GitHub" login, username→repo-picker→AI analysis all work end-to-end on the live public URL. Resume-check not yet tried live (works locally; same auth path, untested against the deployed file-upload path specifically).
- Known limitation: Render's free tier spins the backend down after inactivity - the first request after idle can take 30-60s to wake it up. Worth "warming it up" a minute or two before the actual pitch demo.

**Next Task**
- Wait for approval before starting the next feature.
- The team/org pivot's three planned pieces (commit aggregation, engineering-quality analysis, frontend page) are all done now. No specific next feature queued — check with Yosef before starting anything new.
- Three architectural weaknesses flagged in a staff-engineer-style review (2026-08-02, not yet acted on): (1) all expensive work (`/analyze`, `/resume-check`, the org engineering-report) runs synchronously inside the HTTP request with no background job/queue, so it's exposed to proxy/browser timeouts and has no retry or progress visibility; (2) `rate_limit.py`'s cap is in-memory per-process, so it silently stops being an accurate limit the moment there's more than one backend instance, and there's no idempotency lock against duplicate concurrent runs; (3) no caching/memoization on AI analysis results, so cost and latency scale linearly with repeat requests for the same repo/org instead of being memoized. Also flagged: `list_org_repos`/`fetch_repo_contributors` pagination has no hard ceiling, so a very large real org could tie up a request for a long time and burn through the shared `GITHUB_TOKEN` budget for every user.
- No caching/persistence on the org engineering-report endpoint — re-running the same org re-pays every OpenAI call. Fine for v1 (same trade-off as resume-check), worth reconsidering once managers are actually re-checking the same org repeatedly. (Same issue as the caching point above.)
- Local dev is currently missing a `DATABASE_URL` in `backend/.env` (only the deployed Render backend has one) — needed before more local HTTP testing is practical.
- Resume-check has been verified locally but not yet tried against the live deployed instance.
- Still-open from before the pivot, unresolved: is ~16-17s per repository analysis acceptable, or should the `details` field be trimmed for speed; and is resume-check's stateless (no save/history) design fine long-term.

~~Do a manual visual check of the mobile layout~~ — done, verified live on a real phone against the deployed instance.
~~A Dockerfile would push DevOps score higher~~ — done, existing Dockerfile fixed and now powers the Render deploy.
~~Build the "actual code analysis" half of the team pivot~~ — done, see "Two Features Ago" above.
~~Build the frontend "Team Analysis" page~~ — done, see "Current Feature" above.
...
| Public deployment (Render backend + Postgres, Vercel frontend) | Yosef | Done | (committed directly to `main`) |
| Org-wide contributor commit aggregation (`GET /organizations/{org}/contributors`) | Yosef | Done | (committed directly to `main`) |
| Team/org code-quality & AI-slop analysis (beyond commit counts) | Yosef | Done | (committed directly to `main`) |
| Frontend UI for team/org analysis (Team Analysis page) | Yosef | Done | (committed directly to `main`) |
